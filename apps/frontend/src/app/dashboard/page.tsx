  ...
  return (
    <div className="min-w-0 space-y-6 overflow-x-hidden">
        ...
        <div className="grid gap-6 p-6 md:p-8 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,0.85fr)] xl:items-center">
          <div className="min-w-0">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Button size="lg" variant="outline" onClick={() => void ingestGithubSignals()} disabled={ingesting}>
      ...
      <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-6">
        ...
        <SystemCard icon={RadioTower} title="GitHub Ingestion" value={githubIngestionEnabled ? "Enabled" : "Disabled"} detail={githubConfigured ? "Token configured." : "Unauthenticated mode."} variant={githubIngestionEnabled ? "default" : "muted"} />
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Ingest real GitHub technology signals and run EvolvAI on a live market event.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            {!githubConfigured ? (
            ...
            <Button className="w-full" variant="outline" onClick={() => void ingestGithubSignals()} disabled={ingesting}>
              {ingesting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RadioTower className="mr-2 h-4 w-4" />}
              Ingest GitHub Signals
            </Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {latestLiveEvents.length === 0 ? (
            <p className="text-sm text-muted-foreground">No live external events yet. Ingest GitHub signals or use the reliable demo path.</p>
          ) : null}
          <div className="grid gap-3 xl:grid-cols-2">
            {latestLiveEvents.map((event) => (
              <div key={event.id} className="rounded-lg border bg-background/40 p-4">
                <div className="flex flex-wrap gap-2">
                  <Badge>{event.source === "github" ? "GitHub" : event.source}</Badge>
